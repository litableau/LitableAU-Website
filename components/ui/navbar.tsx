"use client";
import React, { useState } from "react";
import { Menu, X, Pen } from "lucide-react";

export function Navbar() {
  const [isOpen, setIsOpen] = useState(false);

  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-[rgb(23,58,43)] backdrop-blur-md border-b border-[rgb(23,58,43)] shadow-lg">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Desktop Grid layout: left | center | right */}
        <div className="hidden md:grid grid-cols-[1fr_auto_1fr] items-center h-16">
          {/* Left links */}
          <div className="flex items-center justify-start space-x-20">
            <a
              href="/#about"
              className="text-[#f5f5dc] uppercase font-classic text-lg hover:text-[#f5f5dc]/80 transition-colors duration-200"
            >
              ABOUT US
            </a>
            <a
              href="/gallery"
              className="text-[#f5f5dc] uppercase font-classic text-lg hover:text-[#f5f5dc]/80 transition-colors duration-200"
            >
              GALLERY
            </a>
          </div>
          {/* Center Pen Icon */}
          <div className="flex justify-center">
            <a href="/" aria-label="Home">
              <Pen className="h-8 w-8 text-[#f5f5dc] hover:text-[#f5f5dc]/80 transition-colors duration-200" />
            </a>
          </div>
          {/* Right links */}
          <div className="flex items-center justify-end space-x-20">
            <a
              href="/events"
              className="text-[#f5f5dc] uppercase font-classic text-lg hover:text-[#f5f5dc]/80 transition-colors duration-200"
            >
              EVENTS
            </a>
            <a
              href="/contact"
              className="text-[#f5f5dc] uppercase font-classic text-lg hover:text-[#f5f5dc]/80 transition-colors duration-200"
            >
              MEET US
            </a>
          </div>
        </div>

        {/* Mobile Layout: stacked links and centered icon */}
        <div className="md:hidden flex flex-col items-center pt-2 pb-2">
          <div className="flex justify-center mb-2">
            <a href="/" aria-label="Home">
              <Pen className="h-8 w-8 text-[#f5f5dc] hover:text-[#f5f5dc]/80 transition-colors duration-200" />
            </a>
          </div>
        </div>

        {/* Mobile Navigation Drawer */}
        {isOpen && (
          <div className="md:hidden mt-2">
            <div className="px-2 pt-2 pb-3 space-y-1 bg-[rgb(23,58,43)] rounded-lg border border-[#f5f5dc]/20">
              <a
                href="/#about"
                className="block px-4 py-3 text-[#f5f5dc] text-base font-classic uppercase rounded hover:bg-[#f5f5dc]/10 transition-colors duration-200"
                onClick={() => setIsOpen(false)}
                aria-label="About Us"
              >
                ABOUT US
              </a>
              <a
                href="/gallery"
                className="block px-4 py-3 text-[#f5f5dc] text-base font-classic uppercase rounded hover:bg-[#f5f5dc]/10 transition-colors duration-200"
                onClick={() => setIsOpen(false)}
                aria-label="Gallery"
              >
                GALLERY
              </a>
              <a
                href="/events"
                className="block px-4 py-3 text-[#f5f5dc] text-base font-classic uppercase rounded hover:bg-[#f5f5dc]/10 transition-colors duration-200"
                onClick={() => setIsOpen(false)}
                aria-label="Events"
              >
                EVENTS
              </a>
              <a
                href="/contact"
                className="block px-4 py-3 text-[#f5f5dc] text-base font-classic uppercase rounded hover:bg-[#f5f5dc]/10 transition-colors duration-200"
                onClick={() => setIsOpen(false)}
                aria-label="Meet Us"
              >
                MEET US
              </a>
            </div>
          </div>
        )}

        {/* Mobile menu button */}
        <div className="md:hidden absolute top-4 right-4">
          <button
            onClick={() => setIsOpen(!isOpen)}
            className="text-[#f5f5dc] hover:text-[#f5f5dc]/80 transition-colors duration-200 p-2 rounded focus:outline-none focus:ring-2 focus:ring-[#f5f5dc]"
            aria-label={isOpen ? "Close menu" : "Open menu"}
          >
            {isOpen ? <X className="h-6 w-6" /> : <Menu className="h-6 w-6" />}
          </button>
        </div>
      </div>
    </nav>
  );
}
